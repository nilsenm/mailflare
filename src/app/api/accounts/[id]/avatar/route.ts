...
export async function GET(request: Request, { params }: AccountRouteParams) {
	...
	if (!account?.avatarKey) return new Response("No encontrado", { status: 404 });
...
export async function POST(request: Request, { params }: AccountRouteParams) {
	...
	if (!isUploadedAvatarFile(file)) return NextResponse.json({ error: "Falta el archivo de imagen" }, { status: 400 });
	if (!ALLOWED_AVATAR_TYPES.includes(file.type)) return NextResponse.json({ error: "Usa una imagen JPEG, PNG, WebP o GIF" }, { status: 400 });
	if (file.size > MAX_AVATAR_SIZE) return NextResponse.json({ error: "La imagen debe pesar 2 MB o menos" }, { status: 413 });
