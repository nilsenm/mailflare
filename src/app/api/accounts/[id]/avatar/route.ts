...
export async function POST(request: Request, { params }: AccountRouteParams) {
	...
	const { access, account } = await getManagedAccount(request, id);
	if (access.error) return access.error;
	if (!account) return NextResponse.json({ error: translate(dict, "server.accountNotFound") }, { status: 404 });
	const form = await request.formData();
	const file = form.get("file");
	if (!isUploadedAvatarFile(file)) return NextResponse.json({ error: translate(dict, "server.missingImageFile") }, { status: 400 });
	if (!ALLOWED_AVATAR_TYPES.includes(file.type)) return NextResponse.json({ error: translate(dict, "server.useJpegPngWebpGif") }, { status: 400 });
	if (file.size > MAX_AVATAR_SIZE) return NextResponse.json({ error: translate(dict, "server.imageTooLarge") }, { status: 413 });
