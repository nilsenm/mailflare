...
export async function PATCH(request: Request, { params }: CalendarEventRouteParams) {
	...
	if (!existing) return NextResponse.json({ error: translate(dict, "server.eventNotFound") }, { status: 404 });
	...
	if (attendees.length && existing.mailboxId && input.from) { const file = createCalendarInvitation({ ...event, uid: eventId }); await Promise.all(attendees.map((to) => sendEmail(env, { userId: user.id, mailboxId: existing.mailboxId!, from: input.from!, to, subject: `Updated invitation: ${event.title}`, text: event.description || `This event has been updated: ${event.title}.`, attachments: [{ filename: "invite.ics", type: "text/calendar; charset=utf-8", content: file.buffer as ArrayBuffer }] }))); }
