...
export async function POST(request: Request) {
	...
	if (messageIds.length === 0 || !isAllowedBulkMessageAction(payload.action)) {
		return NextResponse.json({ error: "Acción masiva de mensajes no válida" }, { status: 400 });
	}
	...
	if (Object.keys(values).length === 0) {
		return NextResponse.json({ error: "No se solicitaron cambios" }, { status: 400 });
	}
	...
	if (allowedMessageIds.length === 0) {
		return NextResponse.json({ error: "No hay mensajes accesibles" }, { status: 404 });
	}
