...
export async function POST(
	...
	if (!snoozedUntil) {
		return NextResponse.json({ error: translate(dict, "server.chooseFutureSnoozeTime") }, { status: 400 });
	}
	...
	if (!access?.canManage) return NextResponse.json({ error: translate(dict, "server.messageNotFound") }, { status: 404 });
