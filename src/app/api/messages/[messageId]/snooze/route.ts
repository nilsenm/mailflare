...
export async function POST(
	...
	if (!snoozedUntil) {
		return NextResponse.json({ error: "Elige una hora futura para posponer" }, { status: 400 });
	}
	...
	if (!access?.canManage) return NextResponse.json({ error: "Mensaje no encontrado" }, { status: 404 });
