      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            ...
            {item.routingEnabled && <Badge variant="outline">{t("admin.domains.card.routing")}</Badge>}
            {item.sendingEnabled && <Badge variant="outline">{t("admin.domains.card.sending")}</Badge>}
          </div>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={() => loadDns(item.id)}>
            {t("admin.domains.card.dns")}
          </Button>
          <Button
            variant="destructive"
            size="sm"
            ...
            aria-label={t("admin.domains.card.removeAria", { hostname: item.hostname })}
          >
      ...
      {dns && (
        <div className="flex flex-wrap items-center gap-3 text-xs">
          <span className="flex items-center gap-1 text-neutral-500">
            {t("admin.domains.card.routingLabel")}{" "}
          ...
          {dns.routing.missing.length > 0 && (
            <span className="text-red-600 flex items-center gap-1">
              <X className="h-3 w-3" />
              {t("admin.domains.card.missing", { records: dns.routing.missing.join(", ") })}
            </span>
          )}
          <span className="text-neutral-300">|</span>
          <span className="flex items-center gap-1 text-neutral-500">
            {t("admin.domains.card.sendingLabel")}{" "}
