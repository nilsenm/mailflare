	...
	return (
		<details className="mx-6 mb-2 rounded-xl border border-neutral-200 bg-neutral-50 px-4 py-3">
			<summary className="cursor-pointer text-sm font-medium text-neutral-800">
				{score == null ? t("mail.spam.unavailable") : t("mail.spam.score", { score, verdict: verdict ?? t("mail.folders.inbox") })}
			</summary>
			{analysisError ? (
				<p className="mt-2 text-sm text-neutral-600">{t("mail.spam.deliveredNormally")}</p>
			) : parsedSignals.length > 0 ? (
				<div className="mt-3 space-y-1.5 text-sm text-neutral-600">
					<p className="font-medium text-neutral-800">{t("mail.spam.whyScore")}</p>
				...
				<p className="mt-2 text-sm text-neutral-600">{t("mail.spam.noSignals")}</p>
			)}
		</details>
	);
}
